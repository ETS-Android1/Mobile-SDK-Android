...
public class FirmwareUpgradeView extends LinearLayout implements View.OnClickListener, PresentableView {
    ...
    private void initUI(Context context) {
        ...
        mLocationInfoTV = (TextView) findViewById(R.id.tv_location_info);
    ...
    private void initUpgradeData() {
        ...
        showUpgradeLocationInfo(mLocationInfoTV);
        updateComponent();
    ...
    private void getUpgradeLocationInformation() throws InterruptedException {
        int i = 0;
        while(true) {
            TimeUnit.SECONDS.sleep(1);
            i++;
            String info = FlightControllerHandler.getInstance().flightControllerStateResponse(false);
            if (info == null) {
                ToastUtils.showToast(
                        "NO LOCATION");
            }
            ToastUtils.showToast(
                    "Location info: GOT LOCATION");
        }
    }
    ...
    private void showUpgradeLocationInfo(TextView tv) {
        StringBuilder builder = new StringBuilder();
        builder.append("Upgrade Location info:" + "\n");

        String info = FlightControllerHandler.getInstance().flightControllerStateResponse(false);

        if (info != null) {
            builder.append("Location info: " + info);
        } else {
            builder.append("Location info:" + "null" + "\n");
        }
        builder.append("\n");
        ToastUtils.setResultToText(tv, builder.toString());
    }
